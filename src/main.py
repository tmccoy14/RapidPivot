...
VERSION = "0.1.0"
CONTEXT_SETTINGS = dict(auto_envvar_prefix="RPCLI")
HOME = str(Path.home())
...
def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    click.secho("rpcli version: v%s" % VERSION, fg="blue")
    ctx.exit()
...
class RP_CLI(click.MultiCommand):
...
@click.command(cls=RP_CLI, context_settings=CONTEXT_SETTINGS)
@click.option(
    "--home",
...
@click.option(
    ...
    expose_value=False,
    is_eager=True,
    help="Print the current version of rpcli.",
)
@pass_environment
def cli(ctx, verbose, home):
    """
    rpcli is a tool to help sort and format excel spreadsheets.
    """
    ctx.verbose = verbose
    if home is not None:
        ctx.home = home
